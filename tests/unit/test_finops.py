...
def test_estimar_servicos_com_faixa_gratuita_do_bigquery():
    custos = estimar_servicos(
        {
            "glue_dpu_horas": 2,
            "athena_gb": 1024,
            "kinesis_shard_horas": 1,
            "kinesis_put_milhao": 0,
            "lambda_requisicoes_milhao": 0,
            "lambda_gb_segundos": 0,
            "bigquery_tib": 0.5,
        }
    )

    assert custos["glue"] == 0.88
    assert custos["athena"] == 5.0
    assert custos["kinesis"] == 0.015
    assert custos["bigquery"] == 0
